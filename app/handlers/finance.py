...
async def start_income(message: Message, session: AsyncSession, state: FSMContext):
    ...
    await message.answer("Введите сумму дохода (целое число, ₽):", reply_markup=cancel_menu())
...
async def income_amount(message: Message, session: AsyncSession, state: FSMContext):
    ...
    if not amt:
        await message.answer("Нужно целое положительное число. Например: 3500", reply_markup=cancel_menu())
        return
    ...
    if not cats:
        await message.answer("Нет категорий доходов в БД. Обратитесь к владельцу.", reply_markup=main_menu())
        await state.clear()
        return

    await state.set_state(IncomeFlow.category)
    await message.answer("Выберите категорию дохода:", reply_markup=categories_kb([c.name for c in cats]))
...
async def income_category(message: Message, session: AsyncSession, state: FSMContext):
    ...
    await message.answer("Комментарий (опционально). Отправьте текст или '-' чтобы пропустить:", reply_markup=cancel_menu())
...
async def income_confirm(message: Message, session: AsyncSession, state: FSMContext):
    if message.text != "✅ Подтвердить":
        await message.answer("Нажмите ✅ Подтвердить или ❌ Отмена", reply_markup=confirm_menu())
        return
...
async def start_expense(message: Message, session: AsyncSession, state: FSMContext):
    ...
    await message.answer("Введите сумму расхода (целое число, ₽):", reply_markup=cancel_menu())
...
async def expense_amount(message: Message, session: AsyncSession, state: FSMContext):
    amt = parse_amount(message.text)
    if not amt:
        await message.answer("Нужно целое положительное число. Например: 1200", reply_markup=cancel_menu())
        return

    repo = Repo(session)
    _, _, available = await repo.balance()
    if amt > available:
        await message.answer(f"Недостаточно средств. Доступно: {available} ₽", reply_markup=cancel_menu())
        return

    await state.update_data(amount=amt)
    cats = await repo.list_categories(CategoryKind.expense)
    if not cats:
        await message.answer("Нет категорий расходов в БД. Обратитесь к владельцу.", reply_markup=main_menu())
        await state.clear()
        return

    await state.set_state(ExpenseFlow.category)
    await message.answer("Выберите категорию расхода:", reply_markup=categories_kb([c.name for c in cats]))
...
async def reserve_add_start(message: Message, session: AsyncSession, state: FSMContext):
    ...
    await message.answer("Введите сумму для перевода в резерв:", reply_markup=cancel_menu())


@router.message(ReserveFlow.add_amount)
async def reserve_add_amount(message: Message, session: AsyncSession, state: FSMContext):
    amt = parse_amount(message.text)
    if not amt:
        await message.answer("Нужно целое положительное число.", reply_markup=cancel_menu())
        return
    ...
    await repo.add_operation(OperationType.reserve_in, amt, user.id, category_id=None, comment="reserve")
    await state.clear()
    text = await render_balance_message(repo)
...
async def reserve_remove_start(message: Message, session: AsyncSession, state: FSMContext):
    repo = Repo(session)
    user = await repo.get_user_by_tg(message.from_user.id)
    if not user:
        await message.answer("⛔ Доступ запрещён.")
    ...
    await message.answer("Введите сумму для вывода из резерва:", reply_markup=cancel_menu())


@router.message(ReserveFlow.remove_amount)
async def reserve_remove_amount(message: Message, session: AsyncSession, state: FSMContext):
    amt = parse_amount(message.text)
    if not amt:
        await message.answer("Нужно целое положительное число.", reply_markup=cancel_menu())
        return
    ...
    if amt > reserve:
        await message.answer(f"В резерве недостаточно. Сейчас: {reserve} ₽", reply_markup=cancel_menu())
        return

    await repo.add_operation(OperationType.reserve_out, amt, user.id, category_id=None, comment="reserve")
    await state.clear()
    text = await render_balance_message(repo)
