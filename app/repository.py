...
class Repo:
    ...
    async def get_user_by_tg(self, telegram_id: int) -> User | None:
        res = await self.s.execute(select(User).where(User.telegram_id == telegram_id, User.is_active == True))
    ...
    async def list_users(self) -> list[User]:
        res = await self.s.execute(select(User).order_by(User.created_at.asc()))
    ...
    async def list_categories(self, kind: CategoryKind) -> list[Category]:
        res = await self.s.execute(
            select(Category).where(Category.kind == kind, Category.is_active == True).order_by(Category.name.asc())
        )
        return list(res.scalars().all())

    async def get_category_by_name(self, kind: CategoryKind, name: str) -> Category | None:
        res = await self.s.execute(select(Category).where(Category.kind == kind, Category.name == name, Category.is_active == True))
        return res.scalar_one_or_none()

    async def ensure_default_categories(self, income_names: list[str], expense_names: list[str]) -> None:
        for n in income_names:
            if not n:
                continue
            if not await self.get_category_by_name(CategoryKind.income, n):
                self.s.add(Category(kind=CategoryKind.income, name=n.strip(), is_active=True))
        for n in expense_names:
            if not n:
                continue
            if not await self.get_category_by_name(CategoryKind.expense, n):
                self.s.add(Category(kind=CategoryKind.expense, name=n.strip(), is_active=True))
    ...
    async def sum_by_type(self, op_type: OperationType) -> int:
        res = await self.s.execute(select(func.coalesce(func.sum(Operation.amount), 0)).where(Operation.op_type == op_type))
        return int(res.scalar_one())
    ...
    async def list_operations_for_user(self, telegram_id: int, limit: int = 50) -> list[Operation]:
        res = await self.s.execute(
            select(Operation)
    ...
    async def list_operations_filtered(
        ...
        stmt: Select = select(Operation).order_by(Operation.created_at.asc())
        conds = []
        if op_types:
